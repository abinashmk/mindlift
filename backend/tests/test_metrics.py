...
async def test_submit_metrics_success(client: AsyncClient, auth_headers: dict):
    resp = await client.post("/metrics/daily", json=VALID_PAYLOAD, headers=auth_headers)
...
async def test_submit_metrics_unauthenticated(client: AsyncClient):
    resp = await client.post("/metrics/daily", json=VALID_PAYLOAD)
...
async def test_steps_out_of_range_rejected(client: AsyncClient, auth_headers: dict):
    ...
    resp = await client.post("/metrics/daily", json=payload, headers=auth_headers)
    assert resp.status_code == 422


async def test_mood_score_out_of_range_rejected(client: AsyncClient, auth_headers: dict):
    payload = {**VALID_PAYLOAD, "metric_date": "2026-03-11", "mood_score": 6}
    resp = await client.post("/metrics/daily", json=payload, headers=auth_headers)
    assert resp.status_code == 422


async def test_sleep_hours_out_of_range_rejected(client: AsyncClient, auth_headers: dict):
    payload = {**VALID_PAYLOAD, "metric_date": "2026-03-12", "sleep_hours": 25.0}
    resp = await client.post("/metrics/daily", json=payload, headers=auth_headers)
    assert resp.status_code == 422


async def test_heart_rate_below_minimum_rejected(client: AsyncClient, auth_headers: dict):
    payload = {**VALID_PAYLOAD, "metric_date": "2026-03-13", "resting_heart_rate_bpm": 10.0}
    resp = await client.post("/metrics/daily", json=payload, headers=auth_headers)
    assert resp.status_code == 422


async def test_noise_level_above_maximum_rejected(client: AsyncClient, auth_headers: dict):
    payload = {**VALID_PAYLOAD, "metric_date": "2026-03-14", "noise_level_db_avg": 200.0}
    resp = await client.post("/metrics/daily", json=payload, headers=auth_headers)
    assert resp.status_code == 422


async def test_screen_time_minutes_max_ok(client: AsyncClient, auth_headers: dict):
    payload = {**VALID_PAYLOAD, "metric_date": "2026-03-15", "screen_time_minutes": 1440}
    resp = await client.post("/metrics/daily", json=payload, headers=auth_headers)
    assert resp.status_code == 201


async def test_screen_time_minutes_over_max_rejected(client: AsyncClient, auth_headers: dict):
    payload = {**VALID_PAYLOAD, "metric_date": "2026-03-16", "screen_time_minutes": 1441}
    resp = await client.post("/metrics/daily", json=payload, headers=auth_headers)
    assert resp.status_code == 422


async def test_location_home_ratio_out_of_range_rejected(client: AsyncClient, auth_headers: dict):
...
async def test_get_metrics_list(client: AsyncClient, auth_headers: dict):
    ...
    resp = await client.get("/metrics/daily", headers=auth_headers)
...
async def test_get_specific_date(client: AsyncClient, auth_headers: dict):
    ...
    resp = await client.get(f"/metrics/daily/{metric_date}", headers=auth_headers)
    assert resp.status_code == 200
    assert resp.json()["metric_date"] == metric_date


async def test_get_missing_date(client: AsyncClient, auth_headers: dict):
    resp = await client.get("/metrics/daily/2000-01-01", headers=auth_headers)
